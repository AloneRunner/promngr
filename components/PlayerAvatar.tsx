...
export const PlayerAvatar: React.FC<PlayerAvatarProps> = ({ visual, size = 'md' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} relative rounded-full overflow-hidden bg-slate-300 border-2 border-slate-700 shadow-md`}>
      {/* Skin */}
      <div className="absolute inset-0" style={{ backgroundColor: '#FFE0BD' }}></div>

      {/* Clothes/Jersey Neck */}
      <div className="absolute bottom-0 w-full h-1/4 bg-red-800 rounded-t-[50%]"></div>

      {/* Hair */}
      {visual.hairStyle === '1' && <div className="absolute top-0 w-full h-1/3" style={{ backgroundColor: '#3b2219' }}></div>}
      {visual.hairStyle === '2' && <div className="absolute top-0 w-full h-1/4 bg-black/80" style={{ backgroundColor: '#3b2219' }}></div>} {/* Buzz */}
      {visual.hairStyle === '3' && <div className="absolute top-0 left-[-10%] w-[120%] h-1/2 rounded-b-full" style={{ backgroundColor: '#3b2219' }}></div>} {/* Long */}
      {visual.hairStyle === '4' && <div className="absolute top-[-10%] w-full h-1/2 rounded-full" style={{ backgroundColor: '#3b2219' }}></div>} {/* Afro */}
