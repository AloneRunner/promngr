import { MatchEventType, Position, TacticType, TeamMentality } from '../types';
...
const getFieldPlayerFatigueModifiers = (stamina) => {
    if (stamina >= 80) {
...
const getGoalkeeperFatigueModifiers = (stamina) => {
    if (stamina >= 70) {
        // 70-100: Dinç
        return {
            speed: 1.0, strength: 1.0, stamina: 1.0, aggression: 1.0,
...
const getAllFatigueModifiers = (stamina, isGoalkeeper) => {
    return isGoalkeeper
...
const getFatigueModifier = (stamina, type) => {
...
const getEffectiveStat = (player, statName, currentStamina) => {
    const isGK = player.position === Position.GK;
    const mods = getAllFatigueModifiers(currentStamina, isGK);
    const baseStat = player.attributes[statName] || 50;
...
const applyStatFloor = (stat, floor = 40) => {
...
const MAX_PLAYER_SPEED = engineOverrides.MAX_PLAYER_SPEED ?? 1.15; // metre/tick (ölçeklendirilmiş)
const MAX_BALL_SPEED = engineOverrides.MAX_BALL_SPEED ?? 4.4; // metre/tick (ölçeklendirilmiş)
...
const ENABLE_VER_KAC = engineOverrides.ENABLE_VER_KAC ?? true;
// AI Ranges (metre cinsinden - gerçek mesafeler)
const SHOOT_RANGE = engineOverrides.SHOOT_RANGE ?? 32; // 30 → 32m (ceza sahası dışından şut)
const PASS_RANGE_VISION = engineOverrides.PASS_RANGE_VISION ?? 52; // 50 → 52m
const TACKLE_RANGE_BASE = engineOverrides.TACKLE_RANGE_BASE ?? 3.5; // 4.2 → 3.5m (Defans dengeli - ne çok kolay ne çok zor)
const PRESSING_RANGE = engineOverrides.PRESSING_RANGE ?? 18; // 20 → 18m (dengeli pressing)
// Signal / passing tuning (can be overridden from `engineOverrides`)
const SIGNAL_CALL_BONUS = engineOverrides.SIGNAL_CALL_BONUS ?? 80;
const SIGNAL_POINT_BONUS = engineOverrides.SIGNAL_POINT_BONUS ?? 60;
const RUNNING_PASS_BONUS = engineOverrides.RUNNING_PASS_BONUS ?? 100;
const SUPPORT_PROXIMITY_BONUS = engineOverrides.SUPPORT_PROXIMITY_BONUS ?? 40;
const SUPPORT_CALL_EXTRA = engineOverrides.SUPPORT_CALL_EXTRA ?? 60;
const VER_KAC_BONUS = engineOverrides.VER_KAC_BONUS ?? 60;
const VER_KAC_FORWARD_EXTRA = engineOverrides.VER_KAC_FORWARD_EXTRA ?? 20;
const FWD_TO_FWD_BONUS = engineOverrides.FWD_TO_FWD_BONUS ?? 80;
...
const calcPositionScore = (a, position) => {
...
export const calculateBaseOverall = (player, position) => {
...
export const calculateEffectiveRating = (player, assignedPosition, currentCondition = 100) => {
    if (!player || !player.attributes)
        return 60;
    const a = getAttrValues(player.attributes);
    ...
    if (player.position !== assignedPosition) {
        if (player.position === Position.GK || assignedPosition === Position.GK)
            score *= 0.3;
        else if ((player.position === Position.DEF && assignedPosition === Position.FWD) || (player.position === Position.FWD && assignedPosition === Position.DEF))
            score *= 0.7;
        else
            score *= 0.90;
    }
...
export const getFormationStructure = (formation) => {
    switch (formation) {
...
export const getBaseFormationOffset = (formation, role, index, totalInRole) => {
    ...
    if (role === Position.DEF) {
        ...
        if (index === 0)
            return { x: xPos(22), y: spreadY(0, 4, 70) }; // LB 
        if (index === 1)
            return { x: xPos(22), y: spreadY(1, 4, 70) }; // CB
        if (index === 2)
            return { x: xPos(22), y: spreadY(2, 4, 70) }; // CB
        if (index === 3)
            return { x: xPos(22), y: spreadY(3, 4, 70) }; // RB
    ...
    if (role === Position.MID) {
        ...
        if (formation === TacticType.T_4231) {
            if (index < 2)
                return { x: xPos(42), y: index === 0 ? spreadY(0, 2, 44) : spreadY(1, 2, 44) }; // CDM'ler
            if (index === 2)
                return { x: xPos(65), y: spreadY(0, 3, 70) }; // LAM
            if (index === 3)
                return { x: xPos(65), y: PITCH_WIDTH / 2 }; // CAM (merkez)
            if (index === 4)
                return { x: xPos(65), y: spreadY(2, 3, 70) }; // RAM
        }
        if (formation === TacticType.T_4141) {
            if (index === 0)
                return { x: xPos(40), y: PITCH_WIDTH / 2 }; // CDM (merkez)
            return { x: xPos(62), y: spreadY(index - 1, 4, 82) }; // 4 orta saha
        }
        if (formation === TacticType.T_41212) {
            if (index === 0)
                return { x: xPos(40), y: PITCH_WIDTH / 2 }; // CDM
            if (index === 1)
                return { x: xPos(55), y: spreadY(0, 2, 60) }; // LM
            if (index === 2)
                return { x: xPos(55), y: spreadY(1, 2, 60) }; // RM
            if (index === 3)
                return { x: xPos(70), y: PITCH_WIDTH / 2 }; // CAM
        }
        if (formation === TacticType.T_352) {
            if (index < 2)
                return { x: xPos(45), y: spreadY(index, 2, 50) }; // 2 DM
            return { x: xPos(60), y: spreadY(index - 2, 3, 82) }; // 3 AM
        }
        if (formation === TacticType.T_4321) {
            if (index < 3)
                return { x: xPos(48), y: spreadY(index, 3, 70) }; // 3 CM
    ...
    if (role === Position.FWD) {
        ...
        if (formation === TacticType.T_433 || formation === TacticType.T_343) {
            if (index === 1)
                return { x: xPos(80), y: PITCH_WIDTH / 2 }; // ST (merkez) - 88->80
            return { x: xPos(78), y: index === 0 ? spreadY(0, 2, 80) : spreadY(1, 2, 80) }; // Kanatlar - 82->78
        }
        if (totalInRole === 1)
            return { x: xPos(80), y: PITCH_WIDTH / 2 }; // Tek forvet - 88->80
        if (totalInRole === 2)
            return { x: xPos(78), y: index === 0 ? spreadY(0, 2, 44) : spreadY(1, 2, 44) }; // 2 forvet - 86->78
...
export class MatchEngine {
    ...
    DEBUG_INTERVAL = 60; // Her 60 tick'te bir log (1 saniye) - detaylı analiz için
    debugTickCounter = 0; // Global tick sayacı - sıfırlanmaz
    tickCount = 0;
    internalMinute = 0;
    currentLooseBallChaserId = null;
    lastTouchTeamId = null;
    lastShooterId = null;
    lastPossessingTeamId = null;
    // Substitution tracking
    homeSubsMade = 0;
    awaySubsMade = 0;
    MAX_SUBS = 5; // Modern rules allow 5 subs
    lastAISubCheck = 0;
    userTeamId = null; // User's team won't get AI subs
    ...
    homePresserIds = new Set();
    awayPresserIds = new Set();
    ...
    ballStuckTicks = 0;
    lastBallX = PITCH_CENTER_X; // 52.5
    lastBallY = PITCH_CENTER_Y; // 34
    ...
    _cachedHomeStarters = [];
    _cachedAwayStarters = [];
    _starterCacheValid = false;
    _statsLogged = false;
    // === OFSAYT: Defans hatları (her tick güncellenir, actionPass'ta kullanılır) ===
    _homeDefLine = 50;
    _awayDefLine = 55;
    constructor(match, homeTeam, awayTeam, homePlayers, awayPlayers, userTeamId) {
        this.match = match;
        ...
        const filterRestToBench = (players) => {
            let startersCount = 0;
            return players.map(p => {
                if (p.lineup === 'STARTING') {
                    ...
                    if (startersCount > 11) {
                        ...
                        return { ...p, lineup: 'BENCH', lineupIndex: 99 };
                    }
                }
                return p;
            });
        ...
        if (DEBUG_MATCH) {
            ...
            const calcPositionAvg = (players, pos) => {
            ...
            console.log(`\n${'═'.repeat(80)}`);
            console.log(`🏟️  MAÇ BAŞLIYOR: ${homeTeam.name} (${homeAvg}) vs ${awayTeam.name} (${awayAvg})`);
            console.log(`${'═'.repeat(80)}\n`);
            const logTeamTactic = (team, avg, side, gk, def, mid, fwd) => {
                console.log(`📋 ${side}: ${team.name}`);
                console.log(`   Genel Güç: ${avg} | Kadro Detay: KP ${gk}, DF ${def}, OC ${mid}, FW ${fwd}`);
                console.log(`   Formasyon: ${team.tactic.formation} | Stil: ${team.tactic.style}`);
                console.log(`   Mentalite: ${team.tactic.mentality || 'BALANCED'}`);
                console.log(`   Detaylar: Saldırganlık=${team.tactic.aggression}, Tempo=${team.tactic.tempo}, Genişlik=${team.tactic.width}`);
    ...
    initializeTactics(players, tactic) {
        ...
        Object.entries(grouped).forEach(([role, plList]) => {
            ...
            if (role === Position.FWD || role === Position.MID || role === Position.DEF) {
                plList.sort((a, b) => {
                    const scorePos = (p) => {
                        ...
                        if (['SLB', 'LB', 'SLO', 'LM', 'LW', 'LF'].includes(pos))
                            return 1;
                        if (['STP', 'CB', 'MDO', 'CDM', 'MO', 'CM', 'CAM', 'SNT', 'ST', 'CF'].includes(pos))
                            return 2;
                        if (['SĞB', 'RB', 'SĞO', 'RM', 'RW', 'RF'].includes(pos))
                            return 3;
                        return 2;
                    };
            ...
            plList.forEach((p, idx) => {
                if (tactic.customPositions && tactic.customPositions[p.id]) {
                    ...
                    this.baseOffsets[p.id] = getBaseFormationOffset(tactic.formation, role, idx, plList.length);
                }
            });
        });
    }
    getPlayer(id) { return [...this.homePlayers, ...this.awayPlayers].find(p => p.id === id); }
    getPlayerStamina(id) {
        return this.playerStates[id]?.currentStamina;
    }
    // --- COMMUNICATION SYSTEM ---
    emitTeamSignal(from, type, targetId, radius = 45, durationTicks) {
        try {
            ...
            const duration = typeof durationTicks === 'number' ? durationTicks : (engineOverrides.SIGNAL_DURATION ?? 12);
            ...
            teammates.forEach(tm => {
                if (tm.id === from.id)
                    return;
                if (!this.sim.players[tm.id])
                    return;
                if (dist(simFrom.x, simFrom.y, this.sim.players[tm.id].x, this.sim.players[tm.id].y) > radius)
                    return;
    ...
    clearExpiredSignals() {
        Object.keys(this.playerStates).forEach(id => {
            ...
            if (state.incomingSignal && state.incomingSignal.expiryTick <= this.tickCount)
                state.incomingSignal = null;
            if (state.outgoingSignal && state.outgoingSignal.expiryTick <= this.tickCount)
    ...
    updatePlayerSignals() {
        ...
        this.allPlayers.forEach(p => {
            ...
            if (p.id !== this.sim.ball.ownerId && this.sim.players[p.id]) {
                ...
                const pointProb = engineOverrides.POINT_PROB ?? 0.08;
                const callProb = engineOverrides.CALL_PROB ?? 0.06;
    ...
    updateTactic(teamId, newTactic) {
        ...
        if (isHome)
            this.homeTeam.tactic = newTactic;
        else
            this.awayTeam.tactic = newTactic;
    ...
    substitutePlayer(playerIn, playerOutId, isAI = false) {
        ...
        if (outIdx !== -1 && inIdx !== -1) {
            ...
            if (oldPos)
                delete this.sim.players[playerOutId];
    ...
    processAISubstitutions(isHome) {
        ...
        for (const p of starters) {
            if (normalizePos(p) === Position.GK)
                continue; // Don't sub GK unless injured
            const state = this.playerStates[p.id];
            if (!state)
                continue;
        ...
        if (!worstPlayer || worstScore > subThreshold)
            return;
    ...
    syncLineups(homePlayers, awayPlayers) {
    ...
    resetPositions(mode, concedingTeamId) {
        ...
        [...this.homePlayers, ...this.awayPlayers]
            ...
            if (!base)
                return; // Skip if no base offset defined
            let startX = isHome ? base.x : PITCH_LENGTH - base.x;
            let startY = isHome ? base.y : PITCH_WIDTH - base.y;
            if (mode === 'KICKOFF') {
                if (isHome)
                    startX = Math.min(startX, PITCH_CENTER_X - 1);
                else
                    startX = Math.max(startX, PITCH_CENTER_X + 1);
            }
        ...
        if (mode === 'KICKOFF') {
            ...
            enemyTeam.forEach(ep => {
                if (wallCandidates.includes(ep))
                    return; // Barajdakiler zaten yerleşti
                if (!this.sim.players[ep.id])
                    return;
    ...
    updateTeamMentality() {
        ...
        const update = (teamDiff) => {
            if (time > 80 && teamDiff > 0)
                return TeamMentality.PARK_THE_BUS;
            if (time > 75 && teamDiff < 0)
                return TeamMentality.ALL_OUT_ATTACK;
            if (teamDiff > 1)
    ...
    logMatchStats() {
        ...
        console.log(`\n${'═'.repeat(80)}`);
        console.log(`⚽ MAÇ ANALIZ RAPORU: ${h.name} ${this.match.homeScore} - ${this.match.awayScore} ${a.name}`);
        console.log(`${'═'.repeat(80)}\n`);
        // Takım Genel Bilgiler
        console.log(`📋 TAKIMLAR:`);
        console.log(`  ${h.name.padEnd(30)} | Taktik: ${homeTactic.style.padEnd(12)} | Ort. Güç: ${homeAvgStr} | Ort. Hız: ${homeAvgPace}`);
        console.log(`  ${a.name.padEnd(30)} | Taktik: ${awayTactic.style.padEnd(12)} | Ort. Güç: ${awayAvgStr} | Ort. Hız: ${awayAvgPace}`);
        console.log();
        ...
        console.log(`  Şut Sayısı:        ${s.homeShots}`.padEnd(35) + `| ${s.awayShots}`);
        console.log(`  İsabetli Şut:      ${s.homeOnTarget}`.padEnd(35) + `| ${s.awayOnTarget}`);
        console.log(`  Şut Doğruluğu:     %${(s.homeShots > 0 ? (s.homeOnTarget / s.homeShots * 100).toFixed(1) : 0)}`.padEnd(35) + `| %${(s.awayShots > 0 ? (s.awayOnTarget / s.awayShots * 100).toFixed(1) : 0)}`);
        console.log();
        // xG ve Beklentiler
        ...
        console.log(`  Kaleci Kurtarış:   ${s.homeSaves || 0}`.padEnd(35) + `| ${s.awaySaves || 0}`);
        ...
        console.log(`  Gol Verimliliği:   %${homeGoalEff}`.padEnd(35) + `| %${awayGoalEff}`);
        console.log();
        ...
        console.log(`  Saldırganlık: ${homeTactic.aggression.padEnd(12)} | Saldırganlık: ${awayTactic.aggression.padEnd(12)}`);
        console.log(`  Tempo: ${homeTactic.tempo.padEnd(16)} | Tempo: ${awayTactic.tempo.padEnd(16)}`);
        console.log(`  Savunma Hattı: ${homeTactic.defensiveLine.padEnd(9)} | Savunma Hattı: ${awayTactic.defensiveLine.padEnd(9)}`);
    ...
    logHalfTimeStats() {
        ...
        const s = this.match.stats;
        console.log(`\n${'═'.repeat(80)}`);
        console.log(`⏸️  İLK YARI SONU: ${h.name} ${this.match.homeScore} - ${this.match.awayScore} ${a.name}`);
        console.log(`${'═'.repeat(80)}\n`);
        // Sahiplik
        const homePoss = (s.homePossession || 50);
        const awayPoss = (s.awayPossession || 50);
        ...
        console.log(`  Topa Sahiplik:     ${homePoss}%`.padEnd(35) + `| ${awayPoss}%`);
        console.log(`  Şut:               ${s.homeShots} (${s.homeOnTarget} isabetli)`.padEnd(35) + `| ${s.awayShots} (${s.awayOnTarget} isabetli)`);
        console.log(`  Kaleci Kurtarış:   ${s.homeSaves || 0}`.padEnd(35) + `| ${s.awaySaves || 0}`);
    ...
    step() {
        ...
        Object.keys(this.sim.players).forEach(id => {
            const simP = this.sim.players[id];
            const state = this.playerStates[id];
        ...
        if (ballOwnerForPressing) {
            ...
            const sortedDefenders = defendingPlayers.map(p => {
                ...
                .filter(p => {
                if (p.stamina <= 30)
                    return false; // Yorgun eleniyor
                if (p.isBeaten && p.dist > 3.0)
                    return false; // Geçilmiş + uzakta = eleniyor!
                return true;
            })
                ...
                if (a.isBeaten !== b.isBeaten)
                    return a.isBeaten ? 1 : -1;
            ...
            for (const defender of sortedDefenders) {
                if (pressersAdded >= maxPressers)
                    break;
                // Eğer yeterli taze oyuncu varsa, çalımlanmış oyuncuları atla
                if (defender.isBeaten && !useBeatenPlayers)
                    continue;
                if (isDefendingHome) {
        ...
        if (!ballOwner) {
            ...
            allPlayers.forEach(p => {
                ...
                if (p.id === this.currentLooseBallChaserId)
                    effectiveDist -= 3.0;
        ...
        allPlayers.forEach(p => {
            ...
            if (!this.playerStates[p.id]) {
                ...
                this.playerStates[p.id] = {
                    currentStamina: p.condition || 100,
                    decisionTimer: Math.random() * 20, possessionCooldown: 0, actionLock: 0,
                    targetX: base.x, targetY: base.y, momentum: 0, isPressing: false
                };
            }
            const isHome = p.teamId === this.homeTeam.id;
            ...
            if (this.sim.players[p.id].z && this.sim.players[p.id].z > 0) {
                ...
                if (this.sim.players[p.id].z < 0)
                    this.sim.players[p.id].z = 0;
            }
            if (hasBall) {
        ...
        allEvents.sort((a, b) => {
            const priority = (e) => {
                if (e.type === MatchEventType.GOAL)
                    return 0;
                if (e.type === MatchEventType.CARD_RED)
                    return 1;
                if (e.type === MatchEventType.CARD_YELLOW)
                    return 2;
                if (e.type === MatchEventType.FOUL)
                    return 3;
                if (e.type === MatchEventType.FREE_KICK)
                    return 4;
                if (e.type === MatchEventType.CORNER)
                    return 5;
                if (e.type === MatchEventType.KICKOFF)
        ...
        Object.keys(this.playerStates).forEach(id => {
            if (this.sim.players[id]) {
                this.sim.players[id].stamina = this.playerStates[id].currentStamina;
                this.sim.players[id].outgoingSignal = this.playerStates[id].outgoingSignal;
            }
        });
        return {
    ...
    updateBallPhysics() {
        ...
        if (isShotOnGoal) {
            ...
            if (headingToLeftGoal || headingToRightGoal) {
                ...
                if (gk && this.sim.players[gk.id]) {
                    ...
                    if (isCloseRange)
                        gkReachBase = 3.0; // 3.5 -> 3.0 (Yakında daha zor)
                    ...
                    if (distToGK < gkReach && b.z < 3.0) {
                        ...
                        if (gk.playStyles?.includes("Birebir") && isCloseRange) {
                            baseSaveChance += 15; // "Birebir" yeteneği 1v1 nerf'ini kısmen karşılar!
                        }
        ...
        if (b.z < maxPickupHeight) {
            ...
            const nearbyPlayers = [];
            [...this.homePlayers, ...this.awayPlayers].forEach(p => {
                if (!this.playerStates[p.id] || this.playerStates[p.id].possessionCooldown > 0 || !this.sim.players[p.id])
                    return;
                if (p.lineup !== 'STARTING')
                    return;
                ...
                if (d < minD + 2) { // Biraz daha geniş alanda rakip kontrolü
                    ...
                    let jumpPower = (p.attributes.strength || 50) * 0.7 + (p.attributes.positioning || 50) * 0.4;
            ...
            if (closestP) {
                ...
                if (Math.random() * 110 + technique + strengthBonus > difficulty) {
                    ...
                    if (wasOwnTeamTouch && playerRole !== Position.GK && playerRole !== Position.DEF && pPos) {
                        ...
                        if (defLine !== undefined) {
                            ...
                            if (isOffside) {
                                ...
                                this.traceLog.push(`🚩 OFSAYT! ${p.lastName} sahipsiz topu ofsayt pozisyonda aldı!`);
                                this.pendingEvents.push({
                                    minute: this.internalMinute,
                                    type: MatchEventType.OFFSIDE,
                                    description: `🚩 ${p.lastName} ofsayt! Serbest vuruş.`,
                                    teamId: p.teamId,
                                    playerId: p.id
                                });
                                // Rakip takıma ver
                    ...
                    if (!offsideCaught) {
                        ...
                        this.sim.ball.targetId = null; // Hedef artık ulaştı, temizle
                        this.sim.ball.vx = 0;
                        this.sim.ball.vy = 0;
                        this.sim.ball.z = 0;
    ...
    updateGoalkeeperAI(p, isHome) {
        ...
        if (distToBall < 30 && distToBall > 10)
            idealDistFromLine = 5;
        if (distToBall <= 10)
            idealDistFromLine = 3;
    ...
    calculateShotOpening(x, y, targetX, isHome) {
        const dx = targetX - x;
        ...
        for (const e of enemies) {
            ...
            if (d > 25)
                continue; // Çok uzaktakiler önemli değil
    ...
    updateBallCarrierAI(p, isHome, offsideLineX, goalX) {
        if (!this.sim.players[p.id])
            return;
        const simP = this.sim.players[p.id];
        const state = this.playerStates[p.id];
        ...
        if (tactic.tempo === 'Fast')
            ...
            decisionSpeed *= 1.5; // Slower decisions (1.4→1.5)
        ...
        if (playerOVR >= 85) {
            ...
            decisionSpeed *= 0.90; // %10 daha hızlı karar
        }
        ...
        if (state.decisionTimer > decisionSpeed) {
            ...
            const shotOpennessForHold = this.calculateShotOpening(simP.x, simP.y, goalX, isHome);
            ...
            if (distToGoal > 35 && !hasClearPath) { // Kaleye yakın değilse
            ...
            finishingSkill = Math.min(99, Math.round(finishingSkill * FINISH_BUFF_MULT + 1));
            ...
            // === NORMAL PLAY LOGIC ===
                ...
                else if (shotOpenness > 0.5)
                    shootScore += 100;
                else if (shotOpenness > 0.3)
                    shootScore += 30;
                else
                    shootScore -= 80;
                ...
                if (angle > 1.2)
                    ...
                    shootScore -= 40; // Orta açı (52-70 derece)
                ...
                if (distToGoal < 25 && p.playStyles?.includes("Uzaktan Şut"))
                    shootScore += 40;
            }
            // 2. EVALUATE PASSING
            let passScore = 0;
            ...
            if (bestPass) {
                ...
                const shotAngle = Math.abs(Math.atan2(PITCH_CENTER_Y - simP.y, goalX - simP.x));
                if (shotAngle > 0.8 && p.attributes.finishing < 70 && bestPass.score > 100) {
                    passScore += 80;
                    shootScore -= 50;
                }
            }
            ...
            if (spaceObstacles.length === 0)
                dribbleScore += 80; // 30→80 (2.7x): Boş alan = SÜR!
            else
                dribbleScore -= (spaceObstacles.length * 30); // Engel cezası
            // Yetenek etkisi
            ...
            if (pressure > 0 && spaceObstacles.length > 0) {
                ...
                if (nearestDefender) {
                    ...
                    if (mySpeed > defSpeed + 10) {
                        ...
                        if (spaceObstacles.length === 1)
                            dribbleScore += 60; // 30→60 (2x)
                    }
            ...
            if (tactic.style === 'Attacking') {
                ...
                if (this.playerRoles[p.id] === Position.DEF)
            ...
            // 3. POSSESSION (TOPA SAHİP OLMA) - "PAS ODAKLI"
                ...
                if (p.attributes.dribbling > 85)
                    dribbleScore += 70; // 35→70 (2x)
            }
            ...
            if (this.playerRoles[p.id] === Position.DEF || this.playerRoles[p.id] === Position.GK) {
            ...
            if (nearestDefender && nearestDefender.dist < 5) {
                ...
                const hasYellow = this.match.events.some(e => e.type === MatchEventType.CARD_YELLOW && e.playerId === opp.id);
                if (hasYellow) {
            ...
            if (instructions.includes('RoamFromPosition')) {
                ...
                if (this.playerRoles[p.id] === Position.MID || this.playerRoles[p.id] === Position.FWD) {
            ...
            const longShotAttr = p.attributes.longShots || p.attributes.finishing || 50;
            ...
            if (is1v1)
                shootThreshold = Math.min(shootThreshold, 80); // 60 -> 80
            ...
            const isCertainGoal = (distToGoal < 15 && // Çok yakın mesafe
            ...
            if (!isCertainGoal && bestPass && bestPass.type === 'THROUGH' && bestPass.score > 150) {
            ...
            else if (dribbleScore > 0)
                decision = 'DRIBBLE';
            else
                decision = 'PASS';
            if (!is1v1 && Math.random() < 0.1)
                decision = 'DRIBBLE';
            if (decision === 'SHOOT') {
                ...
                simP.state = 'RUN';
                state.shotAttempts = (state.shotAttempts || 0) + 1;
                return;
            }
        ...
        if (isNearEndLine) {
            ...
            if (nearestEnemy) {
                ...
                if (isOnWing && isTechnique) {
                    ...
                    if ((simP.y < PITCH_CENTER_Y && deviation < 0) || (simP.y > PITCH_CENTER_Y && deviation > 0)) {
                        deviation = centerBias;
                    }
                }
                if (simP.y < 5 && deviation < 0)
                    deviation = 15;
                if (simP.y > PITCH_WIDTH - 5 && deviation > 0)
                    deviation = -15;
    ...
    findBestPassOption(p, isHome, offsideLineX, goalX) {
        let bestTarget = null;
        let maxScore = -9999;
        let bestType = 'GROUND';
        let bestTx = 0;
        ...
        if (distToMyGoal < 7 && this.playerRoles[p.id] !== Position.GK)
            return null;
        teammates.forEach(tm => {
            if (tm.id === p.id)
                return;
            ...
            const signalMemoryWindow = engineOverrides.SIGNAL_MEMORY_TICKS ?? 20;
            ...
            opponents.forEach(e => {
                ...
                if (distToLine < 2.5) { // Eğer rakip pas yoluna 2.5m yakınsa
            ...
            // === LONG BALL LOGIC ===
            const isInAttackingThird = isHome ? simP.x > PITCH_LENGTH * 0.66 : simP.x < PITCH_LENGTH * 0.33;
            ...
            opponents.forEach(e => {
                ...
                if (enemyDistToReceiver < 3)
                    receiverPressure += 100;
                // 5m içinde rakip = orta baskı
                else if (enemyDistToReceiver < 5)
                    receiverPressure += 50;
                // 7m içinde rakip = hafif baskı
                else if (enemyDistToReceiver < 7)
                    receiverPressure += 20;
            });
            ...
            if (receiverPressure === 0) {
                ...
                score += 50;
                if (visionStat > 80)
                    score += 30; // İyi görüşü olan bunu ödüllendirir
            ...
            if (scoreDiff < 0)
                forwardBias += 1.5; // Gerideyken daha dikine
            if (tactic.style === 'Possession' && distToGoal < 40)
                forwardBias += 1.5; // Son 1/3'te dikine
            if (forwardProgress > 0)
                score += (forwardProgress * forwardBias);
            ...
            if (tmState?.outgoingSignal?.type === 'POINT') {
                if (forwardProgress > 10)
                    score += 40; // Koşu yoluna at (G Motoru verisi: +40)
            }
            ...
            if (passerRole === Position.FWD && receiverRole === Position.FWD) {
                ...
                if (forwardProgress > 8)
                    score += Math.round(FWD_TO_FWD_BONUS * 0.5);
                // Eğer alıcı koşu halindeyse (hız kontrolü) maksimum bonus
                const tmSpeed = Math.sqrt((simTm.vx || 0) ** 2 + (simTm.vy || 0) ** 2);
                if (tmSpeed > 0.5)
                    score += Math.round(FWD_TO_FWD_BONUS * 0.75);
            }
            ...
            const isWingPlay = tactic.style === 'WingPlay' || (tactic.instructions && tactic.instructions.includes('HitEarlyCrosses'));
            if ((isDeep && isWide) || (isWingPlay && isWide && isInAttackingThird)) {
                ...
                if (isTargetCentral && isTargetDeep) {
                    ...
                    if (tm.attributes.strength > 70 || (tm.playStyles?.includes("Hava Hakimi"))) {
                        score += 200;
                    }
                }
            }
            ...
            if (groundTy < 2 || groundTy > PITCH_WIDTH - 2 || groundTx < 1 || groundTx > PITCH_LENGTH - 1)
                return;
            ...
            const speedMultiplier = runSpeed < 0.7
            ...
            if (tactic.passingStyle === 'Short') {
                if (d < 15)
                    ...
                    score -= 75; // -25→-75 (3x): Uzun pas ÇOK KÖTÜ!
            ...
            // Felsefe: Dikine git. Geriye bakma.
                ...
                if (forwardProgress > 10)
                    score += 90; // 30→90 (3x): İleri oynamak ZORUNLU!
                // Geri pas yasak değil, ama SEVİLMEZ
                if (forwardProgress < -2)
                    score -= 150; // -50→-150 (3x): Geri pas ÇOK KÖTÜ!
            ...
            // Felsefe: En mantıklısını seç.
                ...
                if (d > 10 && d < 35)
                    score += 10; // İdeal futbol mesafesi
                if (forwardProgress > 5)
                    score += 10; // Hafif ileri bias
            }
            let currentBestScore = -9999;
            let currentType = 'GROUND';
            ...
            if (isMakingRun && forwardProgress > 5) {
                ...
                if (boundsCheck && offsideCheck) {
                    ...
                    // Eğer pası atan "Yaratıcı" ise veya "Uzun Top" ustasıysa riski azalt
                    if (p.playStyles?.includes("Yaratıcı") || p.playStyles?.includes("Uzun Topla Pas")) {
                    ...
                    if (isThroughBlocked) {
                        ...
                        const isAggressiveForward = tmRole === Position.FWD && (isMakingRun || isSignaling);
                        if (isAggressiveForward) {
                            throughScore += 75; // 40 + 75 = 115 (Devasa Bonus)
                            if (isSignaling)
                                throughScore += 25; // "Buraya at" diyorsa daha da artır
                            // Vizyonu düşük oyuncular bile bu bariz koşuyu görür
                            if (p.attributes.vision > 60)
                                throughScore += 20;
                        }
                        ...
                        if (passerRole === Position.FWD && receiverRole === Position.FWD) {
                            ...
                            if (tmState?.lastAction === 'PASS' && tmState.lastActionTick) {
                                const tmTicksSincePass = this.tickCount - tmState.lastActionTick;
                                if (tmTicksSincePass < 150) { // 2.5 saniye içinde
            ...
            if (forcedAerialCross || (isDeep && isWide) || (groundRisk > 50 && d > 8 && d < 45) || useChipPass) {
                ...
                else if (isDeep && isWide)
                    aerialScore += 70; // 100 → 70
                ...
                if (tactic.passingStyle === 'Short') {
                    if (useChipPass && d < 20)
                        ...
                        aerialScore -= 40; // Uzun şişirme yasak
                }
                else if (tactic.passingStyle === 'Mixed')
                    aerialScore += 5;
                else if (tactic.passingStyle === 'Direct')
                ...
                if (useChipPass) {
                    ...
                    if (p.attributes.passing > 70)
                        aerialScore += 15; // 30 → 15
                    if (p.attributes.vision > 75)
                        aerialScore += 10; // 20 → 10
                }
    ...
    detectObstacles(p, x, y) {
        const obstacles = [];
        const searchDist = 6;
        this.allPlayers
            .filter(other => other.lineup === 'STARTING')
            .forEach(other => {
            if (other.id === p.id || other.teamId === p.teamId)
                return;
    ...
    findNearestOpponent(p, x, y) {
        let nearest = null;
        let minDist = 999;
        for (const other of this.allPlayers) {
            if (other.id === p.id || other.teamId === p.teamId || other.lineup !== 'STARTING')
                continue;
            const otherPos = this.sim.players[other.id];
            if (!otherPos)
                continue;
    ...
    findNearestEnemyInCone(p, isHome) {
        let nearest = null;
        let minD = 10;
        const simP = this.sim.players[p.id];
        const forwardAngle = isHome ? 0 : Math.PI;
        this.allPlayers
            ...
            if (this.sim.mode !== 'PLAYING' && this.sim.mode !== 'KICKOFF')
                return;
    ...
    applyDefensiveRecoveryLogic(p, state, isHome) {
    ...
    updateOffBallAI(p, isHome, teamHasBall, ballInPlay, offsideLineX, goalX) {
        if (!this.sim.players[p.id])
            return;
        const simP = this.sim.players[p.id];
        const role = this.playerRoles[p.id];
        ...
        if (this.sim.ball.targetId === p.id && !this.sim.ball.ownerId) {
            ...
            if (b.z > 0.5) {
                ...
                const dragFactor = (1 - Math.pow(AIR_DRAG_ESTIMATE, timeToLand)) / (1 - AIR_DRAG_ESTIMATE);
        ...
        const isWinger = role === Position.MID && (this.baseOffsets[p.id]?.y < 20 || this.baseOffsets[p.id]?.y > 48);
        ...
        if (role === Position.DEF)
            widthOffset = 1.0; // 0.5 → 1.0 (etkilenme %50 → %0)
        ...
        if (teamHasBall) {
                ...
                if (role === Position.FWD) {
                    ...
                    if (role === Position.FWD || (role === Position.MID && isAttackingStyle)) {
                        ...
                        if (teamHasBall && ballCarrier && ballCarrier.id !== p.id) {
                            ...
                            if (isAttackingPhase && distToCarrier < 40) { // Topa 40m mesafedeysem ve hücumdaysak
                                ...
                                const isBehindOrLevel = isHome ? simP.x <= carrierPos.x + 5 : simP.x >= carrierPos.x - 5;
                                if (isBehindOrLevel) {
                                    // HEDEF: Top taşıyanın 15-20m önüne geç
                                    const overlapDist = 20;
                                    let targetX_Overlap = isHome ? carrierPos.x + overlapDist : carrierPos.x - overlapDist;
                                    // Ofsayt çizgisiyle sınırla
                                    targetX_Overlap = isHome ? Math.min(targetX_Overlap, offsideLineX - 0.5) : Math.max(targetX_Overlap, offsideLineX + 0.5);
                                ...
                                // Eğer ofsayt çizgisine 2 metreden uzaksam, çizgiye kadar %100 depar at!
                                    ...
                                    const ballChannel = ballY < 23 ? 'LEFT' : (ballY > 45 ? 'RIGHT' : 'CENTER');
                    ...
                    const totalForwards = myTeamPlayers.filter(pl => this.playerRoles[pl.id] === Position.FWD).length;
                    if (totalForwards >= 2) {
                    ...
                    if (!isMakingRun) {
                        if (targetY < areaMinY)
                            targetY = lerp(targetY, areaMinY, 0.35);
                        else if (targetY > areaMaxY)
                            targetY = lerp(targetY, areaMaxY, 0.35);
                        else {
                    ...
                    if (ballCarrierId && ballCarrierId !== p.id) {
                        ...
                        if (carrier && carrierPos) {
                            const carrierDistToGoal = Math.abs(carrierPos.x - (isHome ? PITCH_LENGTH : 0));
                            ...
                            if (carrierDistToGoal < 60) {
                                ...
                                const isAhead = isHome ? simP.x > carrierPos.x : simP.x < carrierPos.x;
                                const isNearCarrier = dist(simP.x, simP.y, carrierPos.x, carrierPos.y) < 30; // 30m içindeysem
                                if (isAhead && isNearCarrier) {
                                    ...
                                    if (ballCentral) {
                                        ...
                                        idealY = simP.y < PITCH_CENTER_Y ? GOAL_Y_TOP : GOAL_Y_BOTTOM;
                                    }
                                    else {
                                        ...
                                        const runToNearPost = (p.id.charCodeAt(0) + this.tickCount) % 10 < 3; // Deterministic Random
                                        if (runToNearPost) {
                    ...
                    if (!isMakingRun) {
                        ...
                        try {
                            if (typeof areaMinY !== 'undefined' && typeof areaMaxY !== 'undefined' && typeof areaCenterY !== 'undefined') {
                                if (targetY < areaMinY)
                                    targetY = lerp(targetY, areaMinY, 0.25);
                                else if (targetY > areaMaxY)
                                    targetY = lerp(targetY, areaMaxY, 0.25);
                                else {
                    ...
                    if (state && state.lastAction === 'PASS' && state.lastActionTick) {
                        ...
                        if (ticksSincePass < 150) {
                            ...
                            const distToGoalLine = Math.abs(simP.x - (isHome ? PITCH_LENGTH : 0));
                            ...
                            speedMod = MAX_PLAYER_SPEED * 1.20; // %120 Hız (Önceden %115)
                    ...
                    const otherForwards = teammates.filter(tm => {
                        if (tm.id === p.id)
                            return false; // Kendim değil
                        if (this.playerRoles[tm.id] !== Position.FWD)
                            return false; // Forvet değilse skip
                        if (tm.lineup !== 'STARTING')
                            return false; // Sadece ilk 11
                        if (!this.sim.players[tm.id])
                            return false;
                        return true;
                    });
                    ...
                    const ballCarrierIsForward = ballCarrierId && otherForwards.some(tm => tm.id === ballCarrierId);
                    if (ballCarrierIsForward && !amIBallOwnerNow && !isMakingRun && isInFinalThirdNow) {
                        // Forvet arkadaşımda top var! YAKLAŞ VE PAS DURUŞU AL!
                        const ballCarrierPos = this.sim.players[ballCarrierId];
                        if (ballCarrierPos) {
                            const distToBallCarrier = dist(simP.x, simP.y, ballCarrierPos.x, ballCarrierPos.y);
                    ...
                    if (isInFinalThird && !amIBallOwner) {
                        ...
                        const nearestOpponent = isHome ? this.awayPlayers : this.homePlayers;
                        let isMarked = false;
                    ...
                    if (isInFinalThird && !amIBallOwner && !isMakingRun) {
                        ...
                        const showFeet = (p.attributes.vision > 60 || p.playStyles?.includes("Bağlantı")) && Math.random() < 0.04;
                        if (showFeet) {
            ...
            if (this.playerRoles[p.id] === Position.FWD && !isMakingRun) {
                try {
                    ...
                    else if (totalForwardsInner === 1)
                        areaCenterYInner = 34;
                    else if (totalForwardsInner === 2)
                        areaCenterYInner = (myIndexInner % 2 !== 0) ? 24 : 44;
                    else {
                    ...
                    if (targetY < areaMinYInner)
                        targetY = lerp(targetY, areaMinYInner, 0.45);
                    else if (targetY > areaMaxYInner)
                        targetY = lerp(targetY, areaMaxYInner, 0.45);
                    ...
                    try {
                        ...
                        if (teamHasBall) {
                            ...
                            if (!isMakingRun)
                                targetX = lerp(targetX, areaPreferredXInner, 0.22);
                        }
                    }
                    catch (e) { /* ignore */ }
                }
            ...
            if (isCreative || hasRoamInstruction) {
                ...
                if (Math.random() < 0.05) { // Frequency reduced 0.1 -> 0.05
            ...
            if (role === Position.FWD) {
                ...
                if (isBallInOwnHalf) {
                    ...
                    if (tactic.pressingIntensity === 'Gegenpress' || tactic.pressingIntensity === 'HighPress') {
                    ...
                    if (myBaseY < 22) {
                        ...
                        if (simP.y > 25)
                            targetY = lerp(targetY, myBaseY, 0.6);
                    }
                    else if (myBaseY > 46) {
                        // Sağ kanat forvet
                    ...
                    if (teamHasBall) {
                        ...
                        const isDeepLying = p.playStyles?.includes("Sırtı Dönük Oyun") || p.playStyles?.includes("False 9");
                        if (isBuildUp && distToBall > 30 && !isPoacher) {
                            ...
                            if (Math.random() < linkUpChance || tactic.style === 'Possession') {
                ...
                if (markingType === 'Man') {
                    ...
                    const nearestThreat = opponents.find(op => {
                        if (!this.sim.players[op.id])
                            return false;
                        const opPos = this.sim.players[op.id];
                        // Benim bölgemde mi? (15m yarıçap)
                        return dist(simP.x, simP.y, opPos.x, opPos.y) < 15 && op.lineup === 'STARTING';
                    });
                ...
                if (role === Position.MID) {
                    ...
                    if (isHome && idealX < minMidX)
                        idealX = minMidX;
                    if (!isHome && idealX > maxMidX)
                        idealX = maxMidX;
                ...
                if (!ballIsClose)
                    shadowStrength *= 0.3; // Uzak toplar için minimal kayma
                ...
                if (role === Position.DEF) {
                    ...
                    teammates.forEach(tm => {
                        if (tm.id === p.id)
                            return;
                        if (this.playerRoles[tm.id] !== Position.DEF)
                            return; // Sadece defans partnerleri
                        const tmState = this.playerStates[tm.id];
                        if (!tmState || !this.sim.players[tm.id])
                            return; // Guard: oyuncu state yoksa atla
                ...
                if (tactic.defensiveLine === 'High')
                    defLineX = isHome ? 37 : 68;
                if (tactic.defensiveLine === 'Deep')
                    defLineX = isHome ? 16 : 89;
                ...
                if (ballCarrierId && role === Position.DEF) {
                    ...
                    if (carrier && carrierPos && carrier.teamId !== p.teamId) {
                        ...
                        if (isCarrierRunningToGoal && carrierDistToGoal < 45) {
                            ...
                            else if (carrierPos.y > 44)
                                threatDirection = 'right';
                            else
                                threatDirection = 'center';
                ...
                if (isGoalBoxEmergency && role === Position.DEF) {
                    ...
                    if (ballCarrierId && (role === Position.DEF || role === Position.MID)) {
                        ...
                        if (ballCarrier && ballCarrier.teamId !== p.teamId) {
                            ...
                            const isEmergencyZone = isHome ? ballX < 35 : ballX > PITCH_LENGTH - 35; // Kendi yarı saham
                            // === FORVET TAKİBİ (Local Initiative) ===
                            // Eğer bir forvet bana doğru geliyorsa, ona KAPAN!
                            let closestThreatToMe = null;
                    ...
                    if (role === Position.GK) {
                        ...
                        if (isDanger) {
                            ...
                            if (distToLine < 20 && this.sim.ball.ownerId) {
                                ...
                                if (ballOwner && ballOwner.teamId !== p.teamId && this.playerRoles[ballOwner.id] === Position.FWD) {
                    ...
                    if (teamHasBall && role === Position.MID && this.sim.ball.ownerId !== p.id) {
                        ...
                        if (isOrganicRunStyle && isInAttackingThirdLR && distToBallLR < 35 && distToBallLR > 5) {
                            ...
                            if (staminaOkLR && simP.state !== 'SPRINT') {
                                ...
                                const myTeamPlayersLR = isHome ? this.homePlayers : this.awayPlayers;
                                ...
                                const nearbyTmLR = myTeamPlayersLR.filter(tm => {
                                    if (tm.id === p.id || !this.sim.players[tm.id])
                                        return false;
                                    return dist(simP.x, simP.y, this.sim.players[tm.id].x, this.sim.players[tm.id].y) < 20;
                                });
                                let channelY;
                                if (nearbyTmLR.length > 2) {
                                    const avgY = nearbyTmLR.reduce((sum, tm) => sum + (this.sim.players[tm.id]?.y || PITCH_CENTER_Y), 0) / nearbyTmLR.length;
    ...
    isClosestTeammateToBall(p) {
        ...
        if (distToGoal < 35 && closerCount < 2)
            return true;
        return false;
    }
    applySteeringBehavior(p, tx, ty, maxSpeed) {
        const simP = this.sim.players[p.id];
        if (!simP)
            return; // Player was removed (red card)
        const state = this.playerStates[p.id];
            ...
            for (let j = i + 1; j < players.length; j++) {
                ...
                if (d < minD && d > 0.01) {
                    ...
                    if (p1HasBall) {
                        ...
                        const p1Dri = (players[i].attributes.dribbling * GLOBAL_BUFF) * (p1State?.currentStamina > 30 ? 1 : 0.8);
                        const p2Tac = players[j].attributes.tackling * (p2State?.currentStamina > 30 ? 1 : 0.8);
                        const diff = p1Dri - p2Tac;
                        if ((p1Dri > 80 && diff > 10) || players[i].playStyles?.includes("Top Cambazı")) {
                            p1.vx *= 0.75;
                            p1.vy *= 0.75; // Less slowdown
                            if (p1State)
                                p1State.actionLock = 4; // Faster recovery
                            if (p2State)
                                p2State.actionLock = 12; // Defender stunned
                        }
                        else {
                            p1.vx *= 0.5;
                            p1.vy *= 0.5;
                            if (p1State)
                                p1State.actionLock = 8;
                    ...
                    if (p2HasBall) {
                        ...
                        if ((p2Dri > 80 && diff > 10) || players[j].playStyles?.includes("Top Cambazı")) {
                            p2.vx *= 0.75;
                            p2.vy *= 0.75;
                            if (p2State)
                                p2State.actionLock = 4;
                            if (p1State)
                                p1State.actionLock = 12;
                        }
                        else {
                            p2.vx *= 0.5;
                            p2.vy *= 0.5;
                            if (p2State)
                                p2State.actionLock = 8;
    ...
    getActionText(teamId) {
        ...
        if (this.sim.ball.x < 32)
            return (teamId === this.homeTeam.id) ? `${teamName} Savunmadan Çıkıyor` : `${teamName} Baskıda`;
        if (this.sim.ball.x > 73)
            return (teamId === this.homeTeam.id) ? `${teamName} Gol Arıyor` : `${teamName} Savunmada`;
        return `${teamName} Topla Oynuyor`;
    }
    actionPass(carrier, target, type, targetOverrideX, targetOverrideY) {
        ...
        if (offsideLineX !== undefined && target.position !== Position.GK) {
            ...
            if (targetIsAhead && !carrierIsAhead && isForwardPass) {
                ...
                this.traceLog.push(`🚩 OFSAYT! ${target.lastName} ofsayt pozisyonda — serbest vuruş!`);
                // Event oluştur
                this.pendingEvents.push({
                    minute: this.internalMinute,
                    type: MatchEventType.OFFSIDE,
                    description: `🚩 ${target.lastName} ofsayt! Serbest vuruş.`,
        ...
        if (targetOverrideX === undefined) {
            ...
            if (type === 'THROUGH') {
                ...
                opponents.forEach(e => {
                    ...
                    if (dline < 2.2 && dist(ex, ey, tx, ty) > 2 && dist(ex, ey, cPos.x, cPos.y) > 2)
                        blockingCount++;
                });
        ...
        else if (teamTactic.passingStyle === 'LongBall')
            styleMod = 2.0; // Çok riskli
        ...
        if (distToT > 30)
            errorMargin *= 1.15; // 25→30, 1.2→1.15
        if (distToT > 40)
            errorMargin *= 1.10; // 35→40, 1.15→1.10
        ...
        if (type === 'AERIAL') {
            ...
            if (distToT < 12) {
                ...
                this.traceLog.push(`${carrier.lastName} uzun bir havadan pas gönderdi!`);
            }
        }
        else {
        ...
        this.sim.ball.targetId = target.id;
        ...
        if (isAttackingRole && !isDefensiveTactic) {
            ...
            if (ENABLE_VER_KAC)
                this.playerStates[carrier.id].supportRunUntil = this.tickCount + runDuration;
    ...
    actionShoot(p, isHome) {
        ...
        const composure = p.attributes.composure * fatigueMods.composure * 1.10;
        const decisions = p.attributes.decisions * fatigueMods.decisions * 1.10;
        // === xG HESABI (BUFF v2: Bitiricilerin Etkisi Artirildi) ===
        const distToGoal = dist(pos.x, pos.y, goalX, PITCH_CENTER_Y);
        const baseXG = Math.max(0.02, (0.60 - (distToGoal / 65)));
        // Hileli bitiricilik bonusu
        const finishingMod = 0.5 + (fin / 100) * 0.90;
        ...
        if (gk && this.sim.players[gk.id]) {
            ...
            if (gkY > PITCH_CENTER_Y)
                ...
                targetY = lerp(PITCH_CENTER_Y, GOAL_Y_BOTTOM - 1, cornerBias);
        }
        else {
            targetY = PITCH_CENTER_Y;
        }
        ...
        if (currentSpeed > MAX_PLAYER_SPEED * 0.5)
            accuracyPenalty += 0.05;
        if (currentSpeed > MAX_PLAYER_SPEED * 0.9)
        ...
        if (is1v1Situation && isForwardShooter) {
            ...
            finalCornerBias = Math.min(0.9, cornerBias + 0.10); // 0.08 -> 0.10
        }
        ...
        if (gk && this.sim.players[gk.id]) {
            ...
            if (gkY > PITCH_CENTER_Y)
                ...
                targetY = lerp(PITCH_CENTER_Y, GOAL_Y_BOTTOM - 1, finalCornerBias);
            dy = targetY - pos.y;
        ...
        // 1. VOLLEY / BICYCLE KICK
            ...
            if (this.sim.players[p.id]) {
                this.sim.players[p.id].shotType = shotType;
                if (this.playerStates[p.id]) {
                    this.playerStates[p.id].shotTypeExpiry = this.tickCount + 45;
                }
            }
        }
        ...
        if (fin >= 95)
            ...
            baseSpread = 0.10 + (85 - fin) * 0.005; // 0.14 -> 0.10
        else if (fin >= 50)
            baseSpread = 0.18 + (70 - fin) * 0.007; // 0.23 -> 0.18
        else
            baseSpread = 0.30 + (50 - fin) * 0.010; // 0.38 -> 0.30
        ...
        if (isPenaltyShot) {
            ...
            if (gk && this.sim.players[gk.id]) {
                const gkY = this.sim.players[gk.id].y;
                if (gkY > PITCH_CENTER_Y)
                    targetY = GOAL_Y_TOP + 1.5;
                else
                    targetY = GOAL_Y_BOTTOM - 1.5;
                dy = targetY - pos.y;
                angle = Math.atan2(dy, dx);
            }
        }
        ...
        if (isOnTarget) {
            if (isHome)
                this.match.stats.homeOnTarget++;
            else
                this.match.stats.awayOnTarget++;
        }
        ...
        if (shotType === 'FREE_KICK') {
            for (const e of enemies) {
                const ePos = this.sim.players[e.id];
                if (!ePos || this.playerRoles[e.id] === Position.GK)
                    continue;
                const d = dist(pos.x, pos.y, ePos.x, ePos.y);
                if (d >= 7 && d <= 13) { // Baraj mesafesi
                    ...
                    if (Math.abs(angleToE - shotAngle) < 0.25) {
                        ...
                        const ballHeightAtWall = vz * timeToWall - 0.5 * GRAVITY * timeToWall * timeToWall;
                        const wallJumpHeight = 1.8 + ((e.attributes.strength || 50) / 100) * 0.5;
                        if (ballHeightAtWall < wallJumpHeight) {
                            if (Math.random() < 0.5) { // %50 Block Chance
    ...
    handleRedCardTactics(teamId, lostPosition) {
    ...
    actionTackle(defender, attacker) {
        if (!attacker)
            return;
        ...
        const attX = attacker.x || this.sim.players[attacker.id]?.x || 0;
        const attY = attacker.y || this.sim.players[attacker.id]?.y || 0;
        const distToMyGoal = dist(attX, attY, myGoalX, 34);
        if (distToMyGoal < 25 && tactic.aggression !== 'Safe') {
            const isLosing = defender.teamId === this.homeTeam.id
                ? this.match.homeScore < this.match.awayScore
                : this.match.awayScore < this.match.homeScore;
            // Kaybetme korkusu veya doğuştan agresiflik
            if (isLosing || tactic.aggression === 'Reckless' || (defender.attributes.aggression || 50) > 85) {
                ...
                if (Math.random() < 0.2) {
                    this.traceLog.push(`‼ ${defender.lastName} kritik müdahale (Panic Faul) riskini aldı!`);
                }
            }
        }
        // TACTICAL BUFF: "Park The Bus" Defense is unbreakable
        if (tactic.style === 'ParkTheBus')
            effectiveDef *= 1.3;
        ...
        if (rollD > rollA) {
            ...
            let foulChance = riskFactor * 0.10; // 0.16 → 0.10
            ...
            if (defender.position === Position.DEF && attackerSim) {
                ...
                if (distToGoal < 25 && Math.random() < 0.05 && tactic.aggression !== 'Safe') { // 30→25m, 0.10→0.05
                    foulChance += 0.20; // 0.40 → 0.20
                    isPanicFoul = true;
                }
            }
            ...
            if (isFoul) {
                ...
                if (tactic.aggression === 'Reckless') {
                    ...
                    attacker.attributes.composure = Math.max(20, attacker.attributes.composure - 8); // Soğukkanlılığı boz
                    // Morale is often not in base interface, safe access
                    if (attacker.morale !== undefined) {
                        attacker.morale = Math.max(20, attacker.morale - 10);
                    }
                    this.traceLog.push(`🤕 ${defender.lastName} sert girdi! ${attacker.lastName} sindirildi.`);
                }
                // Store foul position for free kick
                this.foulPosition = { x: attPos?.x ?? 50, y: attPos?.y ?? 50 };
                // Panic Foul = Almost certanly a card
                if (isPanicFoul) {
                    this.traceLog.push(`🟥 ${defender.lastName} son adam olarak indirdi! (Profesyonel Faul)`);
                }
                ...
                if (isInsideBoxY) {
                    if (defenderTeamIsHome) {
                        ...
                        if (this.foulPosition.x < 16.5)
                            isPenalty = true;
                    }
                    else {
                        // Away defends Right (88.5-105)
                        if (this.foulPosition.x > (105 - 16.5))
                            isPenalty = true;
                    }
                }
    ...
    placeBallForSetPiece(attackingTeam, x, y, buildWall, mode) {
        // Set match mode
        this.sim.mode = mode;
        // Find best taker
        const attackingTeamPlayers = (attackingTeam.id === this.homeTeam.id ? this.homePlayers : this.awayPlayers)
            .filter(p => p.lineup === 'STARTING' && this.playerRoles[p.id] !== Position.GK);
        const taker = attackingTeamPlayers.sort((a, b) => (b.attributes.passing + b.attributes.vision) - (a.attributes.passing + a.attributes.vision))[0];
        if (taker && this.sim.players[taker.id]) {
        ...
        if (!buildWall && (mode === 'PENALTY_HOME' || mode === 'PENALTY_AWAY')) {
            ...
            for (const p of allPlayers) {
                ...
                const distToPenaltySpot = Math.sqrt((simP.x - penaltySpotX) ** 2 + (simP.y - PITCH_CENTER_Y) ** 2);
