...
                case 'Mixed':    dribbleFatigueThreshold = 60;  dribbleFatigueMultiplier = 3;   break; // 1s, normal
                case 'Direct':   dribbleFatigueThreshold = 120; dribbleFatigueMultiplier = 2;   break; // 2s, yavaş ceza
                case 'LongBall': dribbleFatigueThreshold = 45;  dribbleFatigueMultiplier = 3.5; break; // 0.75s, hızlı
            }

            // Attack style modülasyonu — hücum stili toleransı değiştirir
            switch (tactic.style) {
                case 'Attacking':  dribbleFatigueThreshold += 20; break; // Hücumda daha toleranslı
                case 'Counter':    dribbleFatigueThreshold += 30; break; // Kontrada sprint/çalım serbest
...
            // --- DRIBBLE SCORE CAP (bencil dribbler fix 8: 400→250) ---
            dribbleScore = Math.min(dribbleScore, 250);
...
            if (p.playStyles?.includes("Uzun Topla Pas")) effectiveRange += 25;

            if (visionStat > 94) effectiveRange = 200; // Tüm sahayi görür
            else if (visionStat > 84) effectiveRange += 35; // Neredeyse tüm saha
...
            // (forwardProgress calculated above)
			...
			// Dinamik forward bias: daha agresif temel bias (ucuncumotor tuning)
			let forwardBias = 4.5; // Base (daha dikine eğilim)
			switch (tactic.style) {
				case 'Counter':    forwardBias += 1.0; break; // Counter ekstra dikine
				case 'Attacking':  forwardBias += 0.5; break; // Hücumcu ama dengeli
				case 'Possession': forwardBias -= 1.5; break; // Possession daha sabırlı
				case 'Defensive':  forwardBias -= 1.0; break; // Temkinli
				default: /* keep base */ break;
			}
			const scoreDiff = isHome ? (this.match.homeScore - this.match.awayScore) : (this.match.awayScore - this.match.homeScore);
			if (scoreDiff < 0) forwardBias += 1.5; // Gerideyken belirgin şekilde daha dikine
			if (tactic.style === 'Possession' && distToGoal < 40) forwardBias += 1.5; // Son 1/3'te dikine artış

			if (forwardProgress > 0) score += (forwardProgress * forwardBias);

            // === SİNYAL OKUMA SİSTEMİ (G MOTORU ENTEGRASYONU) ===

			// === SİNYAL OKUMA (İyileştirilmiş hafıza + tunelar) ===
			const SIGNAL_MEMORY_TICKS = (engineOverrides.SIGNAL_MEMORY_TICKS as number) ?? 30;
			if (tmState?.incomingSignal?.type === 'CALL') {
				// Eğer sinyal yakın zamanda geldiyse tam bonus, eskidiyse daha hafif bonus
				if (typeof tmState.lastIncomingSignalTick === 'number' && (this.tickCount - tmState.lastIncomingSignalTick) <= SIGNAL_MEMORY_TICKS) {
					score += SIGNAL_CALL_BONUS;
				} else {
					score += Math.round(SIGNAL_CALL_BONUS * 0.6);
				}
			}

			// POINT sinyali: alıcı koşuyu işaretlediyse ve pas ileri gidiyorsa güçlü bonus
			if ((tmState?.incomingSignal?.type === 'POINT' || tmState?.outgoingSignal?.type === 'POINT') && forwardProgress > 10) {
...
            }
			...
			// ANTI-COWARD LOGIC (REFINED - ucuncumotor tuning)
			if (forwardProgress < 0) { // Backward pass
				const pressureForPass = this.detectObstacles(p, simP.x, simP.y).length;
				if (pressureForPass > 0) {
					score += 5; // Baskı altındaysa geri pas akıllı
				} else {
					// Baskı yokken geriye oynamak
					if (tactic.style === 'Possession') {
						score *= 0.95; // Hafif ceza
					} else if (tactic.style === 'Counter') {
						score *= 0.70; // Counter'da sert ceza
					} else {
						score *= 0.85; // Normalde orta ceza
					}
				}
			} else {
				// Forward pass small style bonuses
...
            if (tactic.passingStyle === 'Short') {
                if (d < 15) score += 75;           // 25→75 (3x): Kısa pas ZORUNLU!
                else if (d < 25) score += 30;      // 10→30 (3x): Orta mesafe KABUL EDİLİR
...
                if (forcedAerialCross) aerialScore += 100; // 300 → 100 (hala güçlü ama dominant değil)
...
                if (tactic.passingStyle === 'Short') {
                    if (useChipPass && d < 20) aerialScore -= 10; // Yakın mesafe aşırtma serbest (Şık pas)
...
    private findNearestOpponent(p: Player, x: number, y: number): { player: Player, dist: number } | null {
        let nearest: Player | null = null;
        let minDist = 999;

        for (const other of this.allPlayers) {
            if (other.id === p.id || other.teamId === p.teamId || other.lineup !== 'STARTING') continue;
...
                                    if (this.tickCount % 20 === 0) this.emitTeamSignal(p, 'POINT');
                                }
...
                                if (Math.abs(tm.y - ballSideChannel) < 5) isChannelBlocked = true;
                            }
...
                    if ((role as any) === Position.FWD) targetY = lerp(targetY, PITCH_CENTER_Y, 0.1);
                }
            }
...
                                    const avgY = nearbyTmLR.reduce((sum, tm) =>
                                        sum + (this.sim.players[tm.id]?.y || PITCH_CENTER_Y), 0) / nearbyTmLR.length;
...
        let styleMod = 1.0;
        if (teamTactic.passingStyle === 'Short') styleMod = 0.5;      // Çok isabetli
...
        if (distToT > 40) errorMargin *= 1.10; // 35→40, 1.15→1.10
...
            if (ENABLE_VER_KAC) this.playerStates[carrier.id].supportRunUntil = this.tickCount + runDuration;
...
        if (currentSpeed > MAX_PLAYER_SPEED * 0.9) accuracyPenalty += 0.10;
...
        if (fin >= 95) baseSpread = 0.03 + (100 - fin) * 0.003; // 0.04 -> 0.03
        else if (fin >= 85) baseSpread = 0.06 + (95 - fin) * 0.004; // 0.08 -> 0.06
        else if (fin >= 70) baseSpread = 0.10 + (85 - fin) * 0.005; // 0.14 -> 0.10
        else if (fin >= 50) baseSpread = 0.18 + (70 - fin) * 0.007; // 0.23 -> 0.18
        else baseSpread = 0.30 + (50 - fin) * 0.010; // 0.38 -> 0.30
...
        }
        if (shotType === 'BICYCLE') {
            shotSpeed *= 1.1; spread *= 1.5;
        }
        if (shotType === 'CHIP') {
            shotSpeed *= 0.6; spread *= 0.8;
        }
...
        if (defender.playStyles?.includes("Kayarak Müdahale") || defender.playStyles?.includes("Kayarak Müdahale+")) effectiveDef *= 1.25;
...
        if (tactic.style === 'HighPress') effectiveDef *= 1.1;
	...
	// === TACKLE BALANCE: ucuncumotor tuning ===
	// Apply modest fatigue nerf when defender is tired
	if (defState.currentStamina < 50) effectiveDef *= 0.75;

	// Neutral roll multipliers for fairness
...
                        (attacker as any).morale = Math.max(20, (attacker as any).morale - 10);
                    }
