...
export const SponsorModal: React.FC<SponsorModalProps> = ({ onSelect, t }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-fade-in overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-4xl w-full p-6 md:p-8 shadow-2xl relative flex flex-col my-4 md:my-0">
          
          <div className="text-center mb-6">
              <h2 className="text-3xl font-bold text-white mb-2">{t.sponsorTitle}</h2>
              <p className="text-slate-400">{t.sponsorSelect}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
              {offers.map((offer) => (
                  <div 
                    ...
                    className={`cursor-pointer rounded-xl p-6 border-2 transition-all relative overflow-hidden group ${
                        selectedId === offer.id 
