...
export const TACTICAL_PRESETS: Record<PresetKey, TacticalPreset> = {
  ...
  RouteOne: {
    name: "Route One",
    description:
      "The quickest way to goal is a straight line. Long balls to the target man.",
    tactic: {
      style: "Attacking",
      aggression: "Aggressive",
      tempo: "Fast",
  ...
  ParkTheBus: {
    name: "Park The Bus",
    description:
      "Defend with everyone behind the ball. Frustrate the opponent.",
    tactic: {
      style: "Defensive",
      aggression: "Safe",
      tempo: "Slow",
...
export const validateTactic = (tactic: TeamTactic): TacticWarning[] => {
  ...
  if (
    ...
    (tactic.passingStyle === "LongBall" || tactic.passingStyle === "Direct")
  ) {
    warnings.push({
      type: "WARNING",
      message:
        "Possession style works best with Short or Mixed passing, not Long Ball.",
    });
  }
  ...
  if (tactic.style === "Direct" && tactic.passingStyle === "Short") {
    warnings.push({
      type: "WARNING",
      message:
        "Direct style contradicts Short passing. Try Mixed or Long Ball.",
    });
  }
...
export const analyzeUserHistory = (
  ...
  if (formationFilter) {
    ...
    relevantMatches = relevantMatches.filter(
      (m) =>
        (m.isUserHome ? m.awayTactic?.formation : m.homeTactic?.formation) ===
        formationFilter,
    );
  }
  if (styleFilter) {
    relevantMatches = relevantMatches.filter(
      (m) =>
        (m.isUserHome ? m.awayTactic?.style : m.homeTactic?.style) ===
        styleFilter,
    );
  }
  ...
  const formationStats: Record<
    string,
    { wins: number; draws: number; total: number }
  > = {};
  const styleStats: Record<
    string,
    { wins: number; draws: number; total: number }
  > = {};
  ...
  relevantMatches.forEach((m) => {
    const userTactic =
      m.userFinalTactic || (m.isUserHome ? m.homeTactic : m.awayTactic);
    const userWon = m.userWon;
    const isDraw = !userWon && m.homeGoals === m.awayGoals;

    // Formation
    if (!formationStats[userTactic.formation])
      formationStats[userTactic.formation] = { wins: 0, draws: 0, total: 0 };
  ...
  return {
    ...
    bestFormationWinRate: bestFormationData
      ? Math.round(bestFormationData.winRate)
      : 0,
