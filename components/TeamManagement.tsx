...
const PlayerRow: React.FC<PlayerRowProps> = ({ player, selectedPlayerId, onSelect, onInteractStart, onMove, isFirst, isLast, assignedRole }) => {
    ...
    return (
        <div 
            className={`flex items-center justify-between p-3 rounded cursor-pointer border mb-1 transition-all active:scale-[0.98] ${
...
interface TeamManagementProps {
  ...
  onUpdateLineup: (playerId: string, status: LineupStatus) => void;
...
export const TeamManagement: React.FC<TeamManagementProps> = ({ 
    team, players, onUpdateTactic, onPlayerClick, onUpdateLineup, onSwapPlayers, onMovePlayer, t 
}) => {
  ...
  const handleAutoPick = () => {
      ...
      const { formation } = smartAutoPick(playersCopy, team.tactic.formation);
      handleTacticChange('customPositions', {});
      playersCopy.forEach((p: Player) => {
          onUpdateLineup(p.id, p.lineup);
      });
  };
