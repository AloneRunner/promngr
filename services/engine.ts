...
export const autoPickLineup = (players: Player[], preferredFormation?: TacticType, archetype?: CoachArchetype): { formation: TacticType } => {
    players.forEach(p => { 
        p.lineup = 'RESERVE';
        p.lineupIndex = 99;
    });
    ...
    const gk = sorted.find(p => p.position === Position.GK);
    
    let lineupIdx = 0;
    if(gk) { 
        gk.lineup = 'STARTING'; 
        gk.lineupIndex = lineupIdx++;
    }
    ...
    for(const pos of [Position.DEF, Position.MID, Position.FWD]) {
        const needed = structure[pos];
        const available = sorted.filter(p => p.position === pos && p.lineup === 'RESERVE');
        for(let i=0; i<needed; i++) { 
            if(available[i]) { 
                available[i].lineup = 'STARTING'; 
                available[i].lineupIndex = lineupIdx++;
            } 
        }
    }

    const remaining = sorted.filter(p => p.lineup === 'RESERVE');
    while(lineupIdx < 11 && remaining.length > 0) { 
        remaining[0].lineup = 'STARTING'; 
        remaining[0].lineupIndex = lineupIdx++;
        remaining.shift(); 
    }
    ...
    for(let i=0; i<9; i++) { 
        if(remaining[i]) {
            remaining[i].lineup = 'BENCH'; 
            remaining[i].lineupIndex = benchIdx++;
        }
    }
...
export const generateWorld = (leagueId: string): GameState => {
    ...
    return {
        currentWeek: 1, currentSeason: 2024, userTeamId: teams[0].id, leagueId, leagueId, teams, players, matches: generateSeasonSchedule(teams),
...
export const initializeMatch = (match: Match, homeTeam: Team, awayTeam: Team, homePlayers: Player[], awayPlayers: Player[]) => {
    activeEngine = new MatchEngine(match, homeTeam, awayTeam, homePlayers, awayPlayers);
    return activeEngine.step().liveData.simulation; 
}

export const simulateTick = (match: Match, homeTeam: Team, awayTeam: Team, homePlayers: Player[], awayPlayers: Player[]) => {
...
export const simulateFullMatch = (match: Match, homeTeam: Team, awayTeam: Team, homePlayers: Player[], awayPlayers: Player[]): Match => {
    ...
    match.homeScore = 0; match.awayScore = 0; match.events = [];
    for (let min = 0; min < 90; min += 10) {
        ...
        if (roll < baseGoalProb * (hChance / 1.5)) {
            match.homeScore++;
            match.events.push({ minute: min + getRandomInt(1,9), type: MatchEventType.GOAL, teamId: homeTeam.id, description: `Gol! (${homeTeam.name})` });
        } else if (roll > 1 - (baseGoalProb * (aChance / 1.5))) {
            match.awayScore++;
            match.events.push({ minute: min + getRandomInt(1,9), type: MatchEventType.GOAL, teamId: awayTeam.id, description: `Gol! (${awayTeam.name})` });
        }
    }
    match.isPlayed = true;
...
export const simulateLeagueRound = (gameState: GameState, currentWeek: number): GameState => {
    ...
    matchesToSimulate.forEach(m => {
        ...
        if(home && away) {
            simulateFullMatch(m, home, away, gameState.players.filter(p=>p.teamId===home.id), gameState.players.filter(p=>p.teamId===away.id));
...
export const processWeeklyEvents = (gameState: GameState, t: any) => {
    ...
    const updatedPlayers = gameState.players.map(p => {
        ...
        return { ...p, condition: Math.min(100, p.condition + rec), weeksInjured: Math.max(0, p.weeksInjured - 1), morale: newMorale }
    });
    ...
    if (userTeam) {
        ...
        const maintenance = (userTeam.facilities.stadiumLevel + userTeam.facilities.trainingLevel + userTeam.facilities.academyLevel) * 25000;
        const staffCosts = (userTeam.staff.headCoachLevel + userTeam.staff.scoutLevel + userTeam.staff.physioLevel) * 15000;

        const isHomeWeek = gameState.matches.some(m => m.week === gameState.currentWeek && m.homeTeamId === userTeam.id);
        const ticketIncome = isHomeWeek ? (userTeam.facilities.stadiumCapacity * TICKET_PRICE * (0.5 + Math.random() * 0.5)) : 0;
        const merchandise = (userTeam.reputation * 5) + (teamPlayers.filter(p => p.overall > 85).length * 50000);
        const tvRights = 250000;
        const sponsorIncome = userTeam.sponsor ? userTeam.sponsor.weeklyIncome : 100000;
