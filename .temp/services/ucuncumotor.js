// 'ucuncumotor' implementation file.
// This file contains a full engine implementation (from ucuncumotor.txt).
// Do not re-export `MatchEngine` from another module here — this file defines it.
...
export class MatchEngine {
    ...
    emitTeamSignal(from, type, targetId, radius = 45, durationTicks = 6) {
        try {
    ...
    updateBallCarrierAI(p, isHome, offsideLineX, goalX) {
        ...
        if (state.decisionTimer > decisionSpeed) {
            ...
            if (spaceObstacles.length === 0)
                ...
                dribbleScore -= (spaceObstacles.length * 30); // Engel cezası arttı
            // Yetenek etkisi
            if (p.attributes.dribbling > 60) {
                dribbleScore += ((p.attributes.dribbling - 60) * 1.5);
            }
            // === TAKTİKSEL KİLİT ===
            // Kısa Pas taktiğinde driblingi ÖLDÜR
            if (tactic.passingStyle === 'Short') {
                dribbleScore -= 100; // -30 -> -100 (Artık çalım atmaya korkar)
            }
                ...
                if (nearestDefender) {
                    ...
                    if (mySpeed > defSpeed + 10) {
                        ...
                        if (spaceObstacles.length === 1)
                            dribbleScore += 30; // Önümde sadece o varsa
                    }
            ...
            // === HÜCUM STİLİ VE RİSK YÖNETİMİ (ORGANİK) ===
            // Style artık sadece takım duruşu değil, oyuncunun "Cesaretini" belirler.
            // 1. ATTACKING (HÜCUM) - "CESUR"
            if (tactic.style === 'Attacking') {
                dribbleScore += 20; // Bireysel yetenek göster
                shootScore += 20; // Şut fırsatını kokla
            ...
            // 2. DEFENSIVE (DEFANSİF) - "GARANTİCİ"
                ...
                dribbleScore -= 20; // Risk alma
                shootScore -= 20; // Uzaktan deneme
                passScore += 30; // Topu dolaştır
            ...
            // 3. POSSESSION (TOPA SAHİP OLMA) - "PAS ODAKLI"
                ...
                dribbleScore -= 30; // Çalım top kaybıdır
                passScore += 40; // Pas her şeydir
    ...
    findBestPassOption(p, isHome, offsideLineX, goalX) {
        ...
        teammates.forEach(tm => {
            ...
            if (tmState?.supportRunUntil && tmState.supportRunUntil > this.tickCount) {
            ...
            if (tactic.passingStyle === 'Short') {
                if (d < 15)
                    ...
                    score -= 25; // Uzun pas (35m+) CEZA (ama yasak değil)
            ...
            // Felsefe: Dikine git. Geriye bakma.
                ...
                if (forwardProgress > 10)
                    score += 30; // İleri oynamak BÜYÜK ÖDÜL
                // Geri pas yasak değil, ama SEVİLMEZ
                if (forwardProgress < -2)
                    score -= 50; // Geri pas CİDDİ CEZA
    ...
    updateOffBallAI(p, isHome, teamHasBall, ballInPlay, offsideLineX, goalX) {
        ...
        if (role === Position.DEF)
            widthOffset = 1.0 + (widthOffset - 1.0) * 0.5;
        ...
        if (teamHasBall) {
            ...
            if (distToBall < 50 && role !== Position.GK) {
                ...
                if (role === Position.FWD) {
                    ...
                    if (role === Position.FWD || (role === Position.MID && tactic.mentality === 'Attacking')) {
                    ...
                    if (state && state.lastAction === 'PASS' && state.lastActionTick) {
                        ...
                        if (ticksSincePass < 150) {
                            // HEDEF: RAKİP KALESİ (DÜMDÜZ)
                            const attackDir = isHome ? 1 : -1;
                            // MEVCUT HEDEFLERİ EZ (Override) ve İLERİ FIRLA (Mevcut konumundan 25m ileri)
                            ...
                            // HIZ: MAKSİMUM PATLAMA
                            // Yorulsa bile koşacak.
                            speedMod = MAX_PLAYER_SPEED * 1.15; // %115 Hız (Adrenalin)
            ...
            if (role === Position.FWD) {
                ...
                if (isBallInOwnHalf) {
                    ...
                    // Top tarafına hafif kayma (Compactness) ama abartmadan
                    // Eğer top çok uzaktaysa biraz yanaş (%10), ama dibine girme
                    targetY = lerp(myBaseY, ballY, 0.15);
                    // Eğer kanat forvetse (Base Y < 20 veya > 48), kenarda kalmaya zorla
                    if (myBaseY < 20)
                        targetY = Math.min(targetY, 20); // Sol açık solda kalsın
                    else if (myBaseY > 48)
                        targetY = Math.max(targetY, 48); // Sağ açık sağda kalsın
    ...
    actionTackle(defender, attacker) {
        ...
        if (rollD > rollA) {
            ...
            if (isFoul) {
                ...
                let yellowChance = 0.12; // 0.15 → 0.12
                let redChance = 0.005; // 0.01 → 0.005
                // Penalty içindeki faul (DOGSO - azaltıldı)
                if (isPenalty) {
                    yellowChance += 0.15; // 0.20 → 0.15
                    redChance += 0.05; // 0.10 → 0.05
                }
                if (tactic.aggression === 'Aggressive') {
                    yellowChance = 0.18; // 0.25 → 0.18
                    redChance = 0.015; // 0.03 → 0.015
                }
                else if (tactic.aggression === 'Reckless') {
                    yellowChance = 0.35; // 0.50 → 0.35
                    redChance = 0.08; // 0.15 → 0.08
                }
                else if (tactic.aggression === 'Safe') {
                    yellowChance = 0.06; // 0.08 → 0.06
                    redChance = 0.003; // 0.005 → 0.003
                }
                let cardEvent = null;
