    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fade-in">
        ...
        <div className="bg-slate-800 p-6 rounded-lg border border-slate-700 shadow-xl">
            <h2 className="text-2xl font-bold text-white mb-2 flex items-center gap-2">
                <Activity className="text-emerald-500"/> {t.trainingCenter}
            </h2>
            <p className="text-slate-400 mb-6">{t.trainingDesc}</p>
            ...
            <div className="space-y-4">
                {options.map((opt) => (
                    ...
                    >
                        <div className={`p-3 rounded-full bg-slate-800 ${opt.color}`}>
                            <opt.icon size={24} />
                        </div>
                        <div>
                            <h3 className={`font-bold ${team.trainingFocus === opt.id ? 'text-white' : 'text-slate-300'}`}>{opt.label}</h3>
                            <p className="text-xs text-slate-500 group-hover:text-slate-400">{opt.desc}</p>
                        </div>
                        {team.trainingFocus === opt.id && (
                            <div className="ml-auto text-emerald-500 text-xs font-bold uppercase tracking-widest">Active</div>
                        )}
                    </button>
                ))}
            </div>
