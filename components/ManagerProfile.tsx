...
const MANAGER_UI_COPY = {
    tr: {
        ...
        resetTalents: 'Sıfırla (€250K)', insufficientBalance: 'Yetersiz Bakiye', buyCourse: 'Kursu Al', hired: 'Satın Alındı', hireUpgrade: 'İşe Al / Yükselt',
        ...
        talentCapInfo: 'Şu an dal başına maksimum seviye: {cap}/6. Seviye arttıkça üst katmanlar açılır.',
    ...
    en: {
        ...
        resetTalents: 'Reset (€250K)', insufficientBalance: 'Insufficient Balance', buyCourse: 'Buy Course', hired: 'Purchased', hireUpgrade: 'Hire / Upgrade',
        ...
        talentCapInfo: 'Current maximum per branch: {cap}/6. Higher levels unlock the upper tiers.',
            ...
            <div className="mx-auto max-w-5xl space-y-4 sm:space-y-6">
                                ...
                                return (
                                    <div key={talent.key} className="rounded-xl border border-slate-700 bg-slate-900/40 p-4">
                                        ...
                                        <div className="mt-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                                            ...
                                            >
                                                {isMaxed ? ui.max : talent.level >= talentLevelCap ? replaceTokens(ui.buyCourseLocked, { level: Math.max(2, talent.level * 2 + 1) }) : ui.upgrade}
                                            </button>
                                        </div>
                                    </div>
                                );
                ...
                {managerProfile && (
                    <div className="bg-slate-800/60 rounded-2xl p-4 sm:p-6 border border-slate-600/30">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                            {courses.map((course) => {
                                // PRO_LICENSE is once-only; skill courses are repeatable until talent cap
                                const COURSE_TALENT_MAP: Partial<Record<string, keyof typeof managerProfile.talents>> = {
                                    LEADERSHIP_SEMINAR: 'leadership',
                                    NEGOTIATION_SUMMIT: 'negotiation',
                                    SCOUTING_TOUR: 'scouting',
                                    DEVELOPMENT_WORKSHOP: 'development',
                                };
                                const linkedTalent = COURSE_TALENT_MAP[course.key];
                                const currentTalentLevel = linkedTalent ? (managerProfile.talents?.[linkedTalent] || 0) : null;
                                const isProLicensePurchased = course.key === 'PRO_LICENSE' && (managerProfile.purchasedCourses || []).includes('PRO_LICENSE');
                                const isAtAbsoluteCap = currentTalentLevel !== null && currentTalentLevel >= 6;
                                const isAtLevelCap = currentTalentLevel !== null && currentTalentLevel >= talentLevelCap;
                                const isEffectivelyDone = isProLicensePurchased || isAtAbsoluteCap;
                                const lockedByLevel = (managerProfile.level || 1) < course.minLevel;
                                const lockedByMoney = personalBalance < course.cost;
                                const isDisabled = isEffectivelyDone || isAtLevelCap || lockedByLevel || lockedByMoney;

                                const getButtonLabel = () => {
                                    if (isProLicensePurchased) return ui.hired;
                                    if (isAtAbsoluteCap) return ui.max;
                                    if (isAtLevelCap) return replaceTokens(ui.buyCourseLocked, { level: Math.max(2, (currentTalentLevel! + 1) * 2 - 1) });
                                    if (lockedByLevel) return replaceTokens(ui.buyCourseLocked, { level: course.minLevel });
                                    if (lockedByMoney) return ui.insufficientBalance;
                                    return ui.buyCourse;
                                };

                                return (
                                    <div key={course.key} className={`rounded-xl border ${isProLicensePurchased || isAtAbsoluteCap ? 'border-emerald-500/30 bg-emerald-900/10' : 'border-slate-700 bg-slate-900/40'} p-4`}>
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <div className={`font-bold text-lg ${isEffectivelyDone ? 'text-emerald-400' : 'text-white'}`}>{course.title}</div>
                                                    {isEffectivelyDone && <div className="text-emerald-500 text-sm font-bold">✓</div>}
                                                    {currentTalentLevel !== null && !isAtAbsoluteCap && (
                                                        <div className="text-xs text-slate-400 font-semibold">{currentTalentLevel}/6</div>
                                                    )}
                                                </div>
                                        ...
                                        <div className="mt-4 flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
                                            ...
                                            <button
                                                onClick={() => !isDisabled && onPurchaseCourse(course.key)}
                                                disabled={isDisabled}
                                                className={`px-4 py-2 rounded-lg text-white font-bold transition-all ${
                                                    isEffectivelyDone ? 'bg-emerald-600/50 cursor-not-allowed opacity-80' :
                                                    'bg-gradient-to-r from-amber-600 to-orange-500 disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-400'
                                                }`}
                                            >
                                                {getButtonLabel()}
                                            </button>
